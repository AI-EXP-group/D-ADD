...
class ResNet(nn.Module):
    ...
    def forward(self, x, return_features=False, return_rep=False,return_f1=False):
        x = self.conv1(x)
        x = self.bn1(x)
        out = F.relu(x)
        f1=out = self.layer1(out)

        if(return_f1):
            return f1

        f2=out = self.layer2(out)
        f3=out = self.layer3(out)
        f4=out = self.layer4(out)
        out = F.adaptive_avg_pool2d(out, (1,1))
        rep = out.view(out.size(0), -1)
        out = self.linear(rep)
        if return_rep:
            return out, rep
        elif return_features == False:
            return out
        else:
